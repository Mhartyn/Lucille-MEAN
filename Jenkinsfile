pipeline {
    ...
    stages {
        ...
        stage('BD') {
            steps {
                sh '''
                   docker run -p $PORTBD:27017 -v db:/data/db --network proxy-inverso_nginx_default --name $NAMEBD -e MONGO_INITDB_ROOT_USERNAME=$USERBD -e MONGO_INITDB_ROOT_PASSWORD=$PSW -e MONGO_INITDB_DATABASE=presupuesto -d mongo
                   '''
            }
        }
    }
